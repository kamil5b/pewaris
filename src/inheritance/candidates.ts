...
export function resolveCandidates(
  ...
  const spouse = findSpouse(pewarisId, board, tanggalWarisan)
  if (spouse && !added.has(spouse)) {
    candidates.push({ anggotaId: spouse })
    added.add(spouse)
  }

  const children = findChildren(pewarisId, board, tanggalWarisan)
  for (const childId of children) {
    ...
    const isChildAlive = childAnggota?.tanggalKematian
      ? childAnggota.tanggalKematian > tanggalWarisan
      : true
    ...
    if (isChildAlive) {
      ...
      const grandchildren = findChildren(childId, board, tanggalWarisan)
      for (const gcId of grandchildren) {
        ...
        const isGcAlive = gcAnggota?.tanggalKematian
          ? gcAnggota.tanggalKematian > tanggalWarisan
          : true
